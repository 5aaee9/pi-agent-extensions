...
- Missing OpenAI model limits are filled from pi's catalog for the selected API (`openai-codex` or `openai`). When the host SDK supports it, the extension restores Pi's locally cached runtime catalog offline to pick up newly added models and updated limits/prices, then falls back to the bundled static catalog. This does not refresh catalogs over the network or add models absent from the relay inventory. Missing/unreadable caches and older hosts keep the static fallback. Only fields still unavailable after remote and catalog lookup fall back to a 200,000-token context window and a model-family-specific output limit.
