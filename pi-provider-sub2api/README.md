...
When a relayed Codex stream reports or throws any upstream error, including `Upstream request failed` and `stream_read_error`, the extension retries the complete model request indefinitely. Retry delays start at one second and double after each failure up to a maximum interval of 30 minutes. Cancelling the active turn aborts both the backoff wait and further retries. To prevent partial text or tool calls from a failed attempt leaking into the session, relayed Codex output is released to pi only after an attempt succeeds.
