...
When a relayed Codex stream ends with the exact error `Codex error: stream_read_error` (or the bare code `stream_read_error`), the extension retries the complete model request indefinitely. Retry delays start at one second and double after each failure up to a maximum interval of 30 minutes. Cancelling the active turn aborts both the backoff wait and further retries; spelling/case variants and other Codex errors remain terminal and follow pi's normal error handling. To prevent partial text or tool calls from a failed attempt leaking into the session, relayed Codex output is released to pi only after an attempt reaches a terminal success or non-retryable error.
